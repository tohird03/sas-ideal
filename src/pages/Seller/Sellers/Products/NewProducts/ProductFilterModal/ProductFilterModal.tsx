...
export const ProductFilterModal = observer(() => {
  ...
  const handleModalClose = () => {
    form.resetFields();
    sellerProductStore.setIsOpenNewProductFilterModal(false);
  };
  ...
  const handleSubmit = (values: IGetSellerNewProductParams) => {
    if (values) {
      ...
      sellerProductStore.setNewProductFilterValues(clearedUndifineObj);
    }
    handleModalClose();
  };
  ...
  useEffect(() => {
    form.setFieldsValue(sellerProductStore?.newProductFilterValues);
    if (sellerProductStore?.newProductFilterValues?.tissueId) {
      const selectedTissue = tissueData?.tissueList?.find((el) =>
        el?.id === sellerProductStore?.newProductFilterValues?.tissueId);

      setTissueColors(selectedTissue?.tissueColors as any);
    }

    if (sellerProductStore?.newProductFilterValues?.categoryId) {
      const selectedCategory = productCategoryData?.categoryList?.find((el) =>
        el?.id === sellerProductStore?.newProductFilterValues?.categoryId);
  ...
  return (
    <Modal
      open={sellerProductStore.isOpenNewProductFilterModal}
      onOk={handleModalOk}
