...
import {IGetMainStProductParams} from '@/api/mainStorekeeper/types';
...
export const ProductFilterModal = observer(() => {
  ...
  const handleModalClose = () => {
    form.resetFields();
    sellerProductStore.setIsOpenWarehouseProductFilterModal(false);
  };
  ...
  const handleSubmit = (values: IGetMainStProductParams) => {
    if (values) {
      const clearedUndifineObj = removeUndefinedValues(values);

      sellerProductStore.setWarehouseProductFilterValues(clearedUndifineObj);
    }
    handleModalClose();
  };
  ...
  useEffect(() => {
    form.setFieldsValue(sellerProductStore?.warehouseProductFilterValues);
    if (sellerProductStore?.warehouseProductFilterValues?.tissueId) {
      const selectedTissue = tissueData?.tissueList?.find((el) =>
        el?.id === sellerProductStore?.warehouseProductFilterValues?.tissueId);

      setTissueColors(selectedTissue?.tissueColors as any);
    }

    if (sellerProductStore?.warehouseProductFilterValues?.categoryId) {
      const selectedCategory = productCategoryData?.categoryList?.find((el) =>
        el?.id === sellerProductStore?.warehouseProductFilterValues?.categoryId);
  ...
  return (
    <Modal
      open={sellerProductStore.isOpenWarehouseProductFilterModal}
      onOk={handleModalOk}
