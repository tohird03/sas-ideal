...
export const FilterModal: FC<Props> = observer(
  ({status}) => {
    const [searchOptionsCategory, setSearchOptionsCategory] =
      useState<string>('');
    ...
    const handleModalClose = () => {
      sellerStore.setIsOpenSellerFilterModal(false);
      form.resetFields();
    };
    ...
    const {data: productStatusData, isLoading: productStatusDataLoading} =
      useQuery({
    ...
    const tissueOncChange = (value: string) => {
      ...
      if (value) {
        const selectedTissue = tissueData?.tissueList?.find(
          (el) => el?.id === value
        );
    ...
    const categoryOnChange = (value: string) => {
      ...
      if (value) {
        const selectedCategory = categoryData?.categoryList?.find(
          (el) => el?.id === value
        );
    ...
    //       (el) =>
    //         el?.id ===
    //         storekeeperStore.storekeeperProductFIlterParams?.categoryId
    //     );
    ...
    return (
      <>
        <Modal
          open={sellerStore.isOpenSellerFilterModal}
          onOk={handleModalOk}
        ...
        >
          ...
          >
            ...
            <Form.Item label="Модель" name="modelId" style={{width: '100%'}}>
              <Select
                allowClear
                showSearch
            ...
            <Form.Item label="Ткань" name="tissueId" style={{width: '100%'}}>
              <Select
                allowClear
