...
export const SectionItem = observer((props) => {
  ...
  const {mutate: deleteShowroom} =
    useMutation({
      ...
      mutationFn: (showroomId) => showroomApi.deleteShowroom(showroomId),
      onSuccess: () => {
  ...
  const handleAddNewSeller = () => {
    sellerShowroomsStore.setSingleShowroom(columns);
    sellerShowroomsStore.setIsOpenAddEditSellerModal(true);
  };
  ...
  const handleDeleteWarehouse = () => {
    deleteShowroom(columns?.id);
  };

  const handleGetShowroomUsers = (id) => {
    sellerShowroomsStore.setIsActiveShowroomUsersId(id);
    setPageNumber(pageNumber + 1);
  };

  useEffect(() => {
    if (id === sellerShowroomsStore?.isActiveShowroomUsersId) {
      showroomApi.getShowroomUsers({
        pageSize, pageNumber, showroomId: sellerShowroomsStore?.isActiveShowroomUsersId,
      })
        .then(res => {
          const getUsersData = res?.userList?.map(user => ({
            ...user,
            id: user?.id,
            col_id: sellerShowroomsStore?.isActiveShowroomUsersId,
          ...
          sellerShowroomsStore.setShowroomuser([...sellerShowroomsStore.showroomUsers, ...getUsersData]);
          sellerShowroomsStore.setIsActiveShowroomUsersId(null);
        })
        .catch(addNotification);
    }
  }, [sellerShowroomsStore.isActiveShowroomUsersId]);
  ...
  return (
    ...
    >
      ...
      <div className="kanban-column-list">
        ...
        <Button
          loading={sellerShowroomsStore?.isActiveShowroomUsersId === id}
          icon={<DownOutlined />}
          style={{width: '100%'}}
  ...
  const handleDelete = () => {
    usersApi.revokeRole({
      ...
      .then(() => {
        ...
        sellerShowroomsStore.setShowroomuser([...sellerShowroomsStore.showroomUsers?.filter(el => el?.id !== item?.id)]);
      })
      .catch(addNotification);
  };

  const handleEditSeller = () => {
    sellerShowroomsStore.setSingleUser(item);
    sellerShowroomsStore.setIsOpenAddEditSellerModal(true);
  };
