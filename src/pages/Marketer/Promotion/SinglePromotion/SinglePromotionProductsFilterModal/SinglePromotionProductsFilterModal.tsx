...
import {IPromotionProductListParams} from '@/api/promotion/types';
...
export const SinglePromotionProductsFilterModal = observer(() => {
  ...
  const {data: providerData, isLoading: providerDataLoading} = useQuery({
    ...
    queryFn: () =>
      productManagerApi.getProviders({
        pageNumber: 1,
        pageSize: 50,
      }),
  });
  ...
  const handleModalClose = () => {
    form.resetFields();
    promotionStore.setIsOpenSinglePromotionProductsFilter(false);
  };
  ...
  const handleSubmit = (values: IPromotionProductListParams) => {
    if (values) {
      const clearedUndifineObj = removeUndefinedValues(values);

      promotionStore.setPromotionProductFilter(clearedUndifineObj);
  ...
  useEffect(() => {
    form.setFieldsValue(promotionStore.promotionProductFilter);
    if (promotionStore.promotionProductFilter?.tissueId) {
      const selectedTissue = tissueData?.tissueList?.find((el) =>
        el?.id === promotionStore.promotionProductFilter?.tissueId);

      setTissueColors(selectedTissue?.tissueColors as any);
    }

    if (promotionStore.promotionProductFilter?.categoryId) {
      const selectedCategory = productCategoryData?.categoryList?.find((el) =>
        el?.id === promotionStore.promotionProductFilter?.categoryId);
  ...
  const providerOptions = useMemo(() => (
    providerData?.providerList?.map((el, i) => ({
      value: el?.id,
      label: el?.name,
      key: i,
    }))
  ...
  const handleResetFilter = () => {
    promotionStore.setPromotionProductFilter(null);
    handleModalClose();
  };

  return (
    <Modal
      open={promotionStore.isOpenSinglePromotionProductsFilter}
      onOk={handleModalOk}
    ...
    >
      ...
      >
        ...
        <Form.Item
          ...
          name="tissueColorId"
          style={{width: '100%'}}
          initialValue={promotionStore.promotionProductFilter?.tissueColorId}
        >
          <Select
            allowClear
            showSearch
