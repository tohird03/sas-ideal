...
export const ErrorModal = observer(() => {
  ...
  const handleClose = () => {
    cashierUserStore.setIsHaveSingleUser(null);
    cashierUserStore.setIsOpenWhileAddUserIsHaveUserErrorModal(false);
  };
  ...
  return (
    <Modal
      open={cashierUserStore.isOpenWhileAddUserIsHaveUserErrorModal}
    ...
    >
      <div style={{display: 'flex', flexDirection: 'column'}}>
        <List>
          ...
          >
            <Avatar src={`${imgStages?.apiUrl}${cashierUserStore?.isHaveSingleUser?.avatar}`} icon={<UserOutlined />} />
            <div>
              <p>{cashierUserStore?.isHaveSingleUser?.firstName} {cashierUserStore?.isHaveSingleUser?.lastName}</p>
              <p>{cashierUserStore?.isHaveSingleUser?.phone}</p>
            </div>
          </List.Item>
        </List>
        <div>
        ...
        {isOpenForm &&
          ...
          >
            <Form.Item
              label="Складовщик"
              rules={[{required: !cashierUserStore?.isHaveSingleUser}]}
            >
              <div style={{display: 'flex', flexDirection: 'column'}}>
                {
