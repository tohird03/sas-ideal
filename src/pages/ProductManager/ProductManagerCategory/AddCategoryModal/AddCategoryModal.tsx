...
import {Form, Input, Modal, Select, TreeSelect, TreeSelectProps} from 'antd';
...
export const AddCategoryModal = observer(() => {
  ...
  const {mutate: addCategory} = useMutation({
    ...
    onSuccess: () => {
      ...
      addNotification('Успешно добавлено нового категория');
  ...
  const {mutate: updateCategory} = useMutation({
    ...
    mutationFn: (params: IAddCategory) =>
      categoriesStore.uptadeCategory(params),
    onSuccess: () => {
      queryClient.invalidateQueries({queryKey: ['getCategory']});
      addNotification('Успешно изменять категория');
  ...
  function insertCategoryToParent(
    categories: IGetCategoryList[],
    sub: IGetCategoryList
  ) {
    for (let i = 0; i < categories.length; i++) {
      if (
        !Object.keys(categories[i]).includes('subcategories') &&
        categories[i].id === sub.id
      ) {
        categories[i] = sub;
      }
    }
  ...
  const formatChildCategory: any = (data: IGetCategoryList[], pId?: string) =>
    data?.map((category) => ({
      ...
      children: category.subcategories
        ? formatChildCategory(category.subcategories, category?.id)
        : [],
      data: category,
  ...
  useEffect(() => {
    if (categoriesStore.singleCategory) {
      form.setFieldValue('title', categoriesStore.singleCategory?.title);
      form.setFieldValue(
        'categoryId',
        categoriesStore.singleCategory?.category?.id
      );
    }
    ...
    >
      ...
      >
        <Form.Item
          ...
          rules={[{required: true}]}
        >
          <Input placeholder="Название категории" value={value?.trim()} />
        </Form.Item>
        <Form.Item name="categoryId" label="Выбор дерева">
          <Select
            showSearch
